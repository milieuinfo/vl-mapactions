  ...
  it('bevraagt alle vector sources en voegt alle features toe', (done) => {
   ...
   `;
    ...
    setTimeout(() => {
      ...
      expect(compositeVectorSource.getFeatures()[0].getGeometry().getPolygons()[0].getCoordinates()).to.deep.equal([[[0, 0, 0], [0, 10, 0], [10, 10, NaN], [10, 0, 0], [0, 0, 0]]]);
      expect(compositeVectorSource.getFeatures()[1].getGeometry() instanceof MultiLineString).to.be.true;
      expect(compositeVectorSource.getFeatures()[1].getGeometry().getLineStrings()[0].getCoordinates()).to.deep.equal([[0, 0, 0], [20, 20, 0]]);
      done();
    });
  });
});
