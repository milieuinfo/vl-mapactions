...
describe('map with actions', () => {
  ...
  it('kan maar één actie tegelijkertijd activeren, waarbij enkel deze actie zijn interacties op actief staan', (done) => {
    ...
    afterActivation(() => expect(map.currentAction).to.equal(action1), () => {
      ...
      afterActivation(() => action2.interactions.forEach((interaction) => expect(interaction.getActive()).to.be.true), done);
    });
  });
  ...
  it('er zijn 9 predefined interactions', () => {
    ...
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DragRotate).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DoubleClickZoom).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof KeyboardPan).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof KeyboardZoom).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof MouseWheelZoom).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof PinchZoom).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof PinchRotate).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DragPan).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DragZoom).length).to.equal(1);
  });
  ...
  it('indien gewenst kan de standaard rotation functionaliteit uitgeschaked worden bij het aanmaken van een kaart met acties', () => {
    ...
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DragPan).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DragZoom).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DragRotate).length).to.equal(0);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof PinchRotate).length).to.equal(0);
  });
  ...
  it('indien gewenst kan het zoomen met de mouse wheel afgezet worden', () => {
    ...
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof KeyboardPan).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof KeyboardZoom).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof MouseWheelZoom).length).to.equal(0);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof PinchZoom).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof PinchRotate).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DragPan).length).to.equal(1);
    expect(map.getInteractions().getArray().filter((interaction) => interaction instanceof DragZoom).length).to.equal(1);
